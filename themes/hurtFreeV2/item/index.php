...
<table class="table is-responsive">
	<thead>
		<tr>
			<th scope="col"><?php echo $paginator->sortableColumn('item_id', 'ID Вещи') ?></th>
			<th scope="col"><?php echo $paginator->sortableColumn('name', 'Изображение') ?></th>
			<th scope="col">Название</th>
			<th scope="col">Тип</th>
			<th scope="col">Экипируется</th>
			<th scope="col"><?php echo $paginator->sortableColumn('price_buy', 'Покупка у нпц') ?></th>
			<th scope="col"><?php echo $paginator->sortableColumn('price_sell', 'Продажа нпц') ?></th>
			<th scope="col"><?php echo $paginator->sortableColumn('weight', 'Вес') ?></th>
			<th scope="col"><?php echo $paginator->sortableColumn('attack', 'Атака') ?></th>
			<?php if($server->isRenewal): ?>
			<th scope="col"><?php echo $paginator->sortableColumn('matk', 'МАТК') ?></th>
			<?php endif ?>
			<th scope="col"><?php echo $paginator->sortableColumn('defense', 'Защита') ?></th>
			<th scope="col"><?php echo $paginator->sortableColumn('range', 'Расстояние') ?></th>
			<th scope="col"><?php echo $paginator->sortableColumn('slots', 'Слоты') ?></th>
			<th scope="col"><?php echo $paginator->sortableColumn('refineable', 'Затачиваемая') ?></th>
			<th scope="col"><?php echo $paginator->sortableColumn('cost', 'Cash shop') ?></th>
			<th scope="col"><?php echo $paginator->sortableColumn('origin_table', 'Кастомная') ?></th>
		</tr>
	</thead>
	<tbody>
		...
		<tr>
			...
			<td data-label="Refineable">
				<?php if ($item->refineable): ?>
					<span class="refineable yes">Да</span>
				<?php else: ?>
					<span class="refineable no">Нет</span>
				<?php endif ?>
			</td>
...
<?php else: ?>
<p>Вещи не найдены. <a href="javascript:history.go(-1)">Назад</a>.</p>
<?php endif ?>
