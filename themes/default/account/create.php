...
<p><strong>Примечание:</strong> Перед входом в систему необходимо указать настоящий адрес электронной почты для подтверждения учетной записи.</p>
<?php endif ?>
<p><strong>Примечание:</strong> <?php echo sprintf("Пароль должен содержать от %d до %d символов.", Flux::config('MinPasswordLength'), Flux::config('MaxPasswordLength')) ?></p>
<?php if (Flux::config('PasswordMinUpper') > 0): ?>
<p><strong>Примечание:</strong> <?php echo sprintf(Flux::message('PasswordNeedUpper'), Flux::config('PasswordMinUpper')) ?></p>
<?php endif ?>
<?php if (Flux::config('PasswordMinLower') > 0): ?>
<p><strong>Примечание:</strong> <?php echo sprintf(Flux::message('PasswordNeedLower'), Flux::config('PasswordMinLower')) ?></p>
<?php endif ?>
<?php if (Flux::config('PasswordMinNumber') > 0): ?>
<p><strong>Примечание:</strong> <?php echo sprintf(Flux::message('PasswordNeedNumber'), Flux::config('PasswordMinNumber')) ?></p>
<?php endif ?>
<?php if (Flux::config('PasswordMinSymbol') > 0): ?>
<p><strong>Примечание:</strong> <?php echo sprintf(Flux::message('PasswordNeedSymbol'), Flux::config('PasswordMinSymbol')) ?></p>
<?php endif ?>
<?php if (!Flux::config('AllowUserInPassword')): ?>
<p><strong>Примечание:</strong> <?php echo Flux::message('PasswordContainsUser') ?></p>
<?php endif ?>
